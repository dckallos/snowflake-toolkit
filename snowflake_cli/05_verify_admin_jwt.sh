...
# [<admin>] in ~/.snowflake/connections.toml.
#
...
#      warehouse currently configured under [<admin>]).
...
#   [<admin>].warehouse MUST point at a warehouse that EXISTS in
#   Snowflake when this script runs. The project's initial connections.toml
...
#   [<admin>].warehouse to ARTWORK_WH and re-runs this same
...
# connection defined in ~/.snowflake/connections.toml.
#
# Optional env:
...
CURRENT_WAREHOUSE="$(parse_toml_value "${SNOW_LIB_ADMIN_CONN}" 'warehouse' "${SNOW_LIB_CONNECTIONS_TOML}")"
echo
